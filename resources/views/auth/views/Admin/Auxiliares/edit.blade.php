            ...
            <nav class="navbar bg-light navbar-light">
                ...
                <div class="d-flex align-items-center ms-4 mb-4">
                    ...
                    <div class="navbar-nav w-100">
                        ...
                        <a href="{{route('professoresdb.index')}}" class="nav-item nav-link "></i>Lista de
                            professores</a>
                        <a href="{{route('auxiliaresdb.index')}}" class="nav-item nav-link active"></i>Lista de
                            auxiliares</a>
            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">Alterar Auxiliares</h6>
                    </div>
                    <form method="POST" action="{{route('auxiliaresdb.update', $auxiliar->id)}}">
                        @method('put')
                        @csrf
                        <div class="mb-3">
                            ...
                            <input type="string" data-bs-form="Digite sua senha aqui" class="form-control"
                                ...
                                name="numProcesso" required pattern="^f\d{4}$" value="{{$auxiliar->numProcesso}}">
                            @error('numProcesso')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="exampleInputPassword1" name="nome" required
                                maxlength="100" value="{{$auxiliar->nome}}">
                            @error('nome')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="exampleInputPassword1" min="18" max="99"
                                name="idade" required value="{{$auxiliar->idade}}">
                            @error('idade')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control" id="exampleInputEmail1"
                                aria-describedby="emailHelp" name="email" required value="{{$auxiliar->email}}">
                            @error('email')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <br>
                    <a href="{{route('auxiliaresdb.index')}}"><button type="submit"
