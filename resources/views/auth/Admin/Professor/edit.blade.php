            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-light text-center rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0">Alterar Professor</h6>
                    </div>
                    <form method="POST" action="{{route('professoresdb.update', $professor->id)}}">
                        @method('put')
                        @csrf
                        <div class="mb-3">
                            ...
                            <input type="string" data-bs-form="Digite sua senha aqui" class="form-control"
                                ...
                                name="numProcesso" required pattern="^f\d{4}$" value="{{$professor->numProcesso}}">
                            @error('numProcesso')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="exampleInputPassword1" name="nome" required
                                maxlength="100" value="{{$professor->nome}}">
                            @error('nome')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="exampleInputPassword1" min="1" max="99"
                                name="idade" required value="{{$professor->idade}}">
                            @error('idade')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            <input type="email" class="form-control" id="exampleInputEmail1"
                                aria-describedby="emailHelp" name="email" required value="{{$professor->email}}">
                            @error('email')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
